...
The RPM spec file provided here is for volta-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://volta.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Volta git repository](https://github.com/volta-im/volta-core) but are not part of
...
Currently only `Source10` is used, for the example `volta.conf` file.
...
in the volta git repository. At present only an SVG version of the Volta
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the volta
...
needed when 0.12.1 is released, a proper fix is already in the Volta git
master branch.
...
idea to build Volta Core against a static version of that library compiled
...
The `volta.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `volta-server` package. At this point, most RPM based
...
The `volta-server` package should have SELinux support. How to properly do
...
volta-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/volta/). He has been
packaging Volta for Fedora at least since 2012.
...
4. I renamed the `volta` package that contains the Qt GUI to `volta-core` as
that appears to be how the general population refers to it, in contrast to
`volta-xt` or `volta-classic`. I wanted to make sure the general population
