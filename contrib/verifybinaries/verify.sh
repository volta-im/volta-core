...
###   voltacore.org and volta.org and compares them.
...
WORKINGDIR="/tmp/volta_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://voltacore.org"
HOST2="https://volta.org"
BASEDIR="/bin/"
VERSIONPREFIX="volta-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'volta-',
...
if [ $? -ne 0 ]; then
   echo "volta.org failed to provide signature file, but voltacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "volta.org and voltacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Volta Core binary release signing key installed?"
   fi
