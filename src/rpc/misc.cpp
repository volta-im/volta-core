...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The Volta address to use for the signature.\n"
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "volta-cli and the GUI. There is no server-side difference."
        );
...
UniValue setalgo(const JSONRPCRequest& request)
{
    ...
    throw std::runtime_error(
        ...
        "Uses the same input as commandline parameters or volta.conf\n"
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Volta balance of the wallet\n"
    ...
    obj.push_back(Pair("deprecation-warning", "WARNING: getinfo is deprecated and will be removed in the future."
        " Projects should transition to using getblockchaininfo, getnetworkinfo, and getwalletinfo before upgrading to 2.x"));
