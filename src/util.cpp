...
const char * const VOLTA_CONF_FILENAME = "Volta.conf";
const char * const VOLTA_PID_FILENAME = "voltad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "voltad -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Volta";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VOLTA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", VOLTA_PID_FILENAME)));
}
